...
| `translations`              | Block array of `{translation_iso, name, auto?}`. Populated by `tools/sources/wikidata_names.go` (primary) and `tools/sources/cldr_names.go` (gap-fill). Currently targets `zho`, `jpn`, `hin`, `kor`, `ara`. The optional `auto: true` flag marks automated translations (for future LLM-sourced entries). |                 |
...
| `cldr_names` | `translations[]` (IfMissing per `translation_iso`, fills gaps).                       | Unicode CLDR locale files for `zh`, `ja`, `hi`, `ko`, `ar`. Adds ~330 entries on top of `wikidata_names`. Easily extended to more languages — append to `cldrLocales` in the source. |
| `wikidata_names` | `translations[]` (per-language priority: zh-Hans > zh-CN > zh-SG > zh; ja; hi; ko; ar). | Wikidata SPARQL `rdfs:label` filtered by xml:lang. ~3,700 ISOs covered. Extend by appending to `translationTargets` in the source. |
