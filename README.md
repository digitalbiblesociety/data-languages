...
| `name_zh`        | Simplified Chinese name (zh-Hans). Populated by `tools/sources/wikidata_names.go`, then gap-filled by `tools/sources/cldr_names.go`. |
| `name_zh_auto`   | Sentinel `true` marker indicating `name_zh` was generated by an automated translation rather than a curated upstream. Curated values omit this field. |
...
| `cldr_names` | `name_zh` (IfMissing, fills only gaps).                                              | [Unicode CLDR](https://github.com/unicode-org/cldr-json) `cldr-localenames-full/main/zh/languages.json`. Adds ~70 entries on top of what `wikidata_names` already resolved. |
| `wikidata_names` | `name_zh` (Simplified Chinese, preferring zh-Hans > zh-CN > zh-SG > zh).             | Wikidata SPARQL: `?lang rdfs:label ?label` filtered by xml:lang. ~2,300 ISOs covered. Extensible to other languages — see `nameTargets` in the source. |
