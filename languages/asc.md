---
iso: asc
name: Casuarina Coast Asmat
autonym: Kaweinag
alt_names: [Asmat, "Asmat, Casuarina Coast", As-amat, Matia, Safan, Sapan]
population: 3400
country_id: ID
country_name: Indonesia
location: "Indonesia, Eastern Papua"
area: Papuan language
status_id: 6a
scope: individual
language_type: living
iso_lwc: ind
latitude: -6.43005
longitude: 138.56574
glottocode: casu1237
glottolog_family_id: nucl1709
glottolog_family_name: Nuclear Trans New Guinea
glottolog_classification: "Nuclear Trans New Guinea > Asmat-Awyu-Ok > Asmat-Kamrau Bay > Asmat-Kamoro > Asmat > Casuarina Coast Asmat"
wikidata_id: Q11732046
wikipedia_url: "https://en.wikipedia.org/wiki/Casuarina_Coast_Asmat_language"
translations:
  - translation_iso: ara
    name: اللغة أسمات ساحل كاسواارينا
    auto: true
  - translation_iso: ben
    name: কাসুয়ারিনা কোয়াসত আসমাত
    auto: true
  - translation_iso: deu
    name: Casuarina-Coast-Asmat
    auto: true
  - translation_iso: fra
    name: Asmat de la côte Casuarina
    auto: true
  - translation_iso: hin
    name: कैजुअरीना तट असमत
    auto: true
  - translation_iso: ind
    name: Bahasa Casuarina Coast Asmat
    auto: true
  - translation_iso: jpn
    name: カスアリーナ・コースト・アスマット語
    auto: true
  - translation_iso: kor
    name: 카수아리나 해안 아스마트어
    auto: true
  - translation_iso: por
    name: Asmat da Costa de Casuarina
    auto: true
  - translation_iso: rus
    name: касуарина коаст асматский язык
    auto: true
  - translation_iso: spa
    name: Asmat de la costa Casuarina
    auto: true
  - translation_iso: zho
    name: 红树林海岸阿斯玛特语
    auto: true
rolv_dialects:
  - rolv_code: 1502
    language_tag: asc-x-HIS01502
    name: "Asmat: Manep"
    country_id: ID
    location: "Indonesia, Papua"
  - rolv_code: 1514
    language_tag: asc-x-HIS01514
    name: "Asmat: Shuwru"
    country_id: ID
    location: "Indonesia, Papua"
  - rolv_code: 4111
    language_tag: asc-x-HIS04111
    name: "Asmat: Saman"
    country_id: ID
    location: "Indonesia, Papua"
  - rolv_code: 4231
    language_tag: asc-x-HIS04231
    name: "Asmat: Waganu"
    country_id: ID
    location: "Indonesia, Papua"
  - rolv_code: 7283
    language_tag: asc-x-HIS07283
    name: "Asmat, Casuarina Coast: Matia"
    country_id: ID
    location: "Indonesia, Papua"
  - rolv_code: 7284
    language_tag: asc-x-HIS07284
    name: "Asmat, Casuarina Coast: Sapan"
    country_id: ID
    location: "Indonesia, Papua"
---
Casuarina Coast Asmat is a Papuan language spoken along the Casuarina Coast of South Papua by the Asmat people. It is the most divergent of the Asmat languages.
