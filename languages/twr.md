---
iso: twr
name: Southwestern Tarahumara
autonym: "Rarámuri ra'ícha"
alt_names: [Rocoroibo, South-western, Southwestern, Tarahumara, Tarahumara del Suroeste, Tarahumara nord-occidental, Tarahumara noroccidental, "Tarahumara, Southwestern", Taraumara, Tubare, Vorohio, Western]
population: 100
country_id: MX
country_name: Mexico
status_id: 8a
scope: individual
language_type: living
latitude: 27.03137
longitude: -107.57815
glottocode: sout2971
glottolog_family_id: utoa1244
glottolog_family_name: Uto-Aztecan
glottolog_classification: "Uto-Aztecan > Southern Uto-Aztecan > Tarahumara-Guarijio > Tarahumaran > Unclassified Tarahumaran > Southwestern Tarahumara"
wikidata_id: Q12953909
translations:
  - translation_iso: ara
    name: توراهومارا جنوب غربي
    auto: true
  - translation_iso: ben
    name: দক্ষিণ-পশ্চিম তারাহুমারা
    auto: true
  - translation_iso: deu
    name: Südwestliches Tarahumara
    auto: true
  - translation_iso: fra
    name: tarahumara du Sud-Ouest
  - translation_iso: hin
    name: दक्षिणपश्चिमी तारहुमारा
    auto: true
  - translation_iso: ind
    name: Bahasa Tarahumara Barat Daya
    auto: true
  - translation_iso: jpn
    name: ララムリ・ライチャ語
    auto: true
  - translation_iso: kor
    name: 남서부 타라우마라어
    auto: true
  - translation_iso: por
    name: Tarahumara Sudoeste
    auto: true
  - translation_iso: rus
    name: юго-западный тарахумарский язык
    auto: true
  - translation_iso: spa
    name: Tarahumara del suroeste
    auto: true
  - translation_iso: zho
    name: 西南塔拉乌马拉语
    auto: true
---
