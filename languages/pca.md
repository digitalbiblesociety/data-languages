---
iso: pca
name: Santa Inés Ahuatempan Popoloca
autonym: Popoloca de Santa Inés Ahuatempan
alt_names: [Chocho-Popoloka, Ngigua, Popoloca, "Popoloca, Santa Inés Ahuatempan", Santa Ines Ahuatempan Popoloca]
population: 4000
country_id: MX
country_name: Mexico
status_id: "7"
scope: individual
language_type: living
latitude: 18.38647
longitude: -97.85991
glottocode: sant1454
glottolog_family_id: otom1299
glottolog_family_name: Otomanguean
glottolog_classification: "Otomanguean > Eastern Otomanguean > Popoloca-Zapotecan > Popolocan-Mazatecan > Ixcatec-Chocho-Popolocan > Chocho-Popolocan > Popolocan > Southwestern Popolocan > Tepexi-Zapotitlan > Santa Inés Ahuatempan Popoloca"
wikidata_id: Q42365276
translations:
  - translation_iso: ara
    name: اللغة بوبولوكا سانتا إينيس أهواتيمبان
    auto: true
  - translation_iso: ben
    name: সানতা ইনেস আহুয়াতেমপান পোপোলোকা
    auto: true
  - translation_iso: deu
    name: Santa Inés Ahuatempan-Popoloca
    auto: true
  - translation_iso: fra
    name: popoloca de Santa Inés Ahuatempan
  - translation_iso: hin
    name: सांता इनेस अहुआतेम्पान पोपोलोका
    auto: true
  - translation_iso: ind
    name: Bahasa Santa Inés Ahuatempan Popoloca
    auto: true
  - translation_iso: jpn
    name: サンタ・イネス・アワテンパン・ポポロカ語
    auto: true
  - translation_iso: kor
    name: 산타이네스 아와템판 포폴로카어
    auto: true
  - translation_iso: por
    name: Popoloca de Santa Inés Ahuatempan
    auto: true
  - translation_iso: rus
    name: санта инес ахуатемпан пополокский язык
    auto: true
  - translation_iso: spa
    name: Popoloca de Santa Inés Ahuatempan
    auto: true
  - translation_iso: zho
    name: 圣内斯阿瓦坦潘波波洛卡语
    auto: true
rolv_dialects:
  - rolv_code: 15753
    language_tag: pca-x-HIS15753
    name: "Popoloca, Santa Inés Ahuatempan: Todos Santos Almolonga"
    country_id: MX
    location: "Mexico, Puebla"
---
