---
iso: npl
name: Southeastern Puebla Nahuatl
autonym: Southeastern Puebla Nahuatl
alt_names: [Eastern Nawa, Mexicano, Nahua de la Huasteca, Nahuatl, "Nahuatl, Southeastern Puebla", Náhuatl du Huasteca, Náhuatl del Sureste de Puebla, Southeastern Puebla, Tehuacán Náhuatl, Tehuacán Valley Nahuatl, Náhuatl de Sureste de Puebla, Náhuatl del Valle de Tehuacán]
population: 92000
country_id: MX
country_name: Mexico
location: Southern Central Mexico
area: Nahuatl variety of Mexico
status_id: "5"
scope: individual
language_type: living
latitude: 18.39688
longitude: -97.12655
glottocode: sout2973
glottolog_family_id: utoa1244
glottolog_family_name: Uto-Aztecan
glottolog_classification: "Uto-Aztecan > Southern Uto-Aztecan > Corachol-Aztecan > Aztec > Western Nahuatl > Central Nahuatl > Tlaxcala-Southeastern Puebla Nahuatl > Southeast Puebla-Northern Oaxaca Nahuatl > Southeastern Puebla Nahuatl"
wikidata_id: Q4632950
translations:
  - translation_iso: ara
    name: ناواتل بويبلا الجنوبية الشرقية
    auto: true
  - translation_iso: ben
    name: দক্ষিণ-পূর্ব পুএবলা নাহুয়াতল
    auto: true
  - translation_iso: deu
    name: Südöstliches Puebla-Nahuatl
    auto: true
  - translation_iso: fra
    name: nahuatl du Puebla du Sud-Est
  - translation_iso: hin
    name: दक्षिण-पूर्वी पुएब्ला नवात्ल
    auto: true
  - translation_iso: ind
    name: Bahasa Puebla Nahuatl Tenggara
    auto: true
  - translation_iso: jpn
    name: 南東プエブラ・ナワトル語
    auto: true
  - translation_iso: kor
    name: 남동부 푸에블라 나와틀어
    auto: true
  - translation_iso: por
    name: náuatle de Puebla Sudeste
    auto: true
  - translation_iso: rus
    name: юго-восточный пуебла нахуатльский язык
    auto: true
  - translation_iso: spa
    name: Náhuatl del Sureste de Puebla
    auto: true
  - translation_iso: zho
    name: 东南普埃布拉纳瓦特尔语
    auto: true
---
Tehuacan–Zongolica a.k.a. Southeastern Puebla Nahuatl is a variety of Nahuatl spoken by ethnic Nahua people in southeastern Puebla state (Tehuacan) and southern Veracruz (Zongolica) in Mexico.
