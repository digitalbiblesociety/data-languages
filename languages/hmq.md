---
iso: hmq
macrolanguage_id: hmn
name: Eastern Qiandong Miao
autonym: Qiandong Miao
alt_names: [Black Miao, Central Miao, Eastern East-Guizhou Miao, Heh Miao, Hei, "Hmong, Eastern Qiandong", Hmu, Miao, "Miao, Eastern Qiandong", Chientung Miao, Eastern Hmu, Hei Miao, mo˧]
population: 350000
country_id: CN
country_name: China
location: Southern China
area: Hmongic language spoken in China
status_id: 6a
scope: individual
language_type: living
latitude: 26.49663
longitude: 109.04627
glottocode: east2370
glottolog_family_id: hmon1336
glottolog_family_name: Hmong-Mien
glottolog_classification: "Hmong-Mien > Hmongic > Nuclear Hmongic-Ho Ne > Nuclear Hmongic > East Hmongic > Northeastern Qiandongic Miao > Eastern Qiandongic Miao > Eastern Qiandong Miao"
wikidata_id: Q27431369
translations:
  - translation_iso: ara
    name: لغة ميــاو تشياندونغ الشرقية
    auto: true
  - translation_iso: ben
    name: পূর্ব কিয়ানদোং মিয়াও
    auto: true
  - translation_iso: deu
    name: Östliches Qiandong-Miao
    auto: true
  - translation_iso: fra
    name: Miao du Qiandong oriental
    auto: true
  - translation_iso: hin
    name: पूर्वी क्यानडोंग मियाओ
    auto: true
  - translation_iso: ind
    name: Bahasa Qiandong Miao Timur
    auto: true
  - translation_iso: jpn
    name: 東部黔東苗語
  - translation_iso: kor
    name: 동부 첸둥 먀오어
    auto: true
  - translation_iso: por
    name: Miao Qiandong Oriental
    auto: true
  - translation_iso: rus
    name: восточный киандонг миаоский язык
    auto: true
  - translation_iso: spa
    name: Miao Qiandong oriental
    auto: true
  - translation_iso: zho
    name: 东部土语
---
The Hmu language, also known as Qiandong Miao, Central Miao (中部苗语), East Hmongic, or Black Miao, is a dialect cluster of Hmongic languages of China. The best studied dialect is that of Yǎnghāo (养蒿) village, Taijiang County, Guizhou Province.
